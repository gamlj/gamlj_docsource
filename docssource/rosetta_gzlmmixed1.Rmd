...
`r keywords("`r jamovi()`, R, SPSS, logistic, generalized mixed, mixed models")`

`r version("2.0.0")` `r draft` 


Here you can find comparisons of results obtained in `r jamovi()` GAMLj,R, and SPSS. When not explicitly discussed, the code of different software is written with the aim of obtaining identical results across programs (not necessarily with the most efficient strategy).
...
`r jamovi()` GAMLj module for `Generalized mixed models` can be used to estimate a random coefficient mixed model. First, it requires choose the specific model we want to run: the `Logistic` model.
