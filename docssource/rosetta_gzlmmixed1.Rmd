...
Here you can find comparisons ofresults obtained in jamovi GAMLj and R. When not explicitely discussed, the code of different software is written with the aim of obtaing identical results across programs (not necessarely with the most efficient strategy).

In this example we compare GAMLj results with R results obtained by IDRE on their [tutorial web page](https://stats.idre.ucla.edu/r/dae/mixed-effects-logistic-regression/)

# The research design

In this example, we are going to explore lung cancer remission using a simulated dataset, which we have posted online. A variety of outcomes were collected on patients, who are nested within doctors, who are in turn nested within hospitals. There are also a few doctor level variables, such as Experience that we will use in our example ([IDRE](https://stats.idre.ucla.edu/r/dae/mixed-effects-logistic-regression/)). Data can be downloaded [here](https://stats.idre.ucla.edu/stat/data/hdp.csv)

# R
The researcher is interested in studying the relationships between cancer remission and both patients and doctors carateristics. The analyst uses the R `glmer` command from the `lme4 Package`  to estimate a mixed effects logistic regression model with `Il6`, `CRP`, and `LengthofStay` as patient level continuous predictors, `CancerStage` as a patient level categorical predictor (I, II, III, or IV), `Experience` as a doctor level continuous predictor, and a random intercept by `DID`, doctor ID.
...
and finaly, the $e^B$ are computed.
...
jamovi GAMLj module for `Generalized mixed models` can be used to estimate a random coefficient mixed model. First, it requires choose the specific model we want to run: the `Logistic` model.
...
Please note that we put `LengthOfStay` in covariates because it is a continuous variable, even though jamovi reconizes it a categorical. By putting it in `Covariates`, we implicitely ask the module to treat it a continuous variable. The opposite holds for `DID` (doctors ID) which jamovi reconizes as continuous but the module treats as categorical after we put it in the `Clusters` field.
...
For the predictors coefficients we have substantially the same results as in R, apart from rounding. For the intercept  we have different results. The reason is the different default coding of the variables in GAMLj and in R. In R default, factors are coded with the _dummy coding scheme_, that is 
...
Thus, in GAMLj the intectept is the expected mean of the (transformed) dependent variable, whereas in R is the expected mean of the (trasformed) dependent variable for all continuous independent variables equal to zero and the group scoring all zeros in the categorical variable.
...
The IDRE analysis did not report the omnibus tests. In R, one can obtain the ominibus tests employing the `car::Anova` command. The command lets us specify what Type of analysis to do. We can ask of the Type III analysis and we get the Chi-square tests. 
...
Compared with GAMLj results, there's substantial agreement even though the figures are not the same, apart from rounding. The reason is that the estimation of the deviance of the model, on which the omnibus tests are based, may be sligtly influenced by the scale of the variables. It is well-known that in the general linear model (i.e. regression), the linear coefficients are invariant to linear scaling of the independent variables. That is, if you have centered or not centered variables the B's are the same (when no interaction is present in the model). In generalized mixed models, the scale may slightly influence the estimation, yielding different results for rescaled variables. 

Thus, the small difference in the omnibus tests obtained with R and with GAMLj are due to the fact that in the R analysis the variables are not rescaled, while in GAMLj they are centered. In fact, if we do not center the variables in GAMLj, results become identical to the R results.


<img src="rosetta/gzlmmixed/results_chi2.png" class="img-responsive" alt="">

Notice, however, that the conclusion of the analyses are equivalent no matter the scale of the variables.

# SPSS

In SPSS we can run a logistic mixed models with `Generalized mixed models` menu. 

<img src="rosetta/gzlmmixed/spss_results.png" class="img-responsive" alt="">

However, the `Generalized mixed models` module does not allow to specify whether a variable should be considered as continuous or categorical, so we have to define variables types in the `Variable view` tab of the SPSS spreadsheet window. 

<img src="rosetta/gzlmmixed/spss_scale.png" class="img-responsive" alt="">

We want to be sure that `LengthOfStay`, `IL6`, `CRP` and `Experience` are set as `Scale`, whereas `DID` and `CancerStage` should be `Nominal`.

The we can go to the mixed model menu. 

<img src="rosetta/gzlmmixed/spss_menu.png" class="img-responsive" alt="">

First, we should specify the cluster variable, by dragging `DID` into the `Subject` field, on the right of the `Canvas` panel.
...
Here we need to flag the `Include intercept` options and select `DID` from the menu `Subject combination`. This means _allow intercepts to vary  across DID_.
...
As regards the fixed effects coefficients, we notice that the continuous predictors show B's that are equivalent to R and jamovi estimates, but with opposite sign. The intercept and the categorical predictor are clearly differ. Those differences are due to way SPSS picks the reference level of the dummy coding for variables. In R and jamovi, the reference level is the first level, given standard ordering of the variable level (such as the cardinal order or the alphabetical one). SPSS picks the last group! Thus, here we are predicting the probability of _not remission_, that is `remission=0`. That explain the opposite sign. For the categorical independent variable `CancerStage`, SPSS picks the group `CancerStage=IV` as refrence group, whereas GAMLj and R pick `CancerStage=I`. That explains the differences in the parameters.

In SPSS generalized mixed models we can change the reference group in the `Build Options` tab, selecting `Descending` in the `Sorting order` options.
...
As regards the random coefficients variance is quite different from the R or GAMLj results.
