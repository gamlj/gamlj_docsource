...
we can see that it seems to be a negative relationship between the two variables. A Simple regression ((in jamovi `Linear Models` -> `GAMLj::General Linear Model`)) confirms this impression.
...
Because a simple regression line has two coefficients (the constant term and the slope) we can let the intercept (or constant term) to vary across clusters, the slope, or both. Practically, we define the intercept, or the slope (of `beer`), or both as random coefficients.
