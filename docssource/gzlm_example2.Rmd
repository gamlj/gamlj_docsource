...
Skipping the intercept (recall that nobody interprets the intercept :-) ), the first coefficient is `Sex1`: B=0.896. The `exp(B)` is 2.449. This means that the odd of reportin `Pain=yes` over `Pain=No` for men (see `Contrast` column) is 2.499 times larger than for the averge of the sample (`(M,F)`).
