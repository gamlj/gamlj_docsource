...
Some people may find this strange, because of the common belief that two overlapping confidence intervals should correspond to a non-significant difference. Howerver, this belief is wrong.
...
Skipping the intercept (recall that nobody interprets the intercept :-) ), the first coefficient, `Sex1`. The `exp(B)` is 2.449. This means that the odd of reportin `Pain=yes` over `Pain=No` for men (see `Contrast` column) is 2.499 times larger than for the averge of the sample (`(M,F)`).
This conparison may sound awkward, but the awkwardness can be fixed. Recall that GAMLj default coding for factors is `deviations`, meaing that factors are coded such that each group (but the first) is compared with the average of the sample. When only two groups are present, a better coding is `simple` that "simply" compares the two groups. We can set the conding in `Factor Coding`.
...
The B and exp(B) parameters associated with `Sex` have changed, although the z-test and p-value did not. The odd-ratio associated with `Sex` is not exp(B)=6, and the contrast says `M-F`- This means that the odd of reporting the pain is 6 times larger for men than for women.  
...
As suspected, there is an interaction between `Treatment` and `Age`, so it means that the efficacy of the treatments dependens on the patient's age. The plot will tell us how. 
