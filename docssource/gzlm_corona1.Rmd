---
title: "GzLM: Polynomial Poisson regression for coronavirus data"
...
ii<-grep("United Kindom",data$Country.Region,fixed=T)
...
`r keywords("Polynomial regression, Poisson distribution, Generalized linear model")`
...
In the input panel, we need to select a `custom model`, because we need to set the `Poisson` as the DV distribution and the `Log` as the link function. 
...
The fit of the same model estimation (polynomial up to the 4th grade) is shown in the next section for all countries with more than 10 days of data. 
