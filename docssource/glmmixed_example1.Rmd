...
`r datafile("cvs file","school_exam.csv")` and open it in [jamovi](https://www.jamovi.org/download.html). Be sure to install the new version of GAMLj module from within jamovi library. Data are similated for educational purposes, and should be used only for exercising.
...
Each schools has different number of students, ranging from `r min(table(data$school))` to `r max(table(data$school))`. Each student presents three variable values: the score in the `math` test, the number of `activity` undertaken and whether the exam was passed `pass=1` or not, `pass=0`.


Here are the frequency table for the `pass` variable and an abridged table for the schools variable. `frequencies` -> `Contingency tables`.
...
We can now define the variables role in the model, by selecting the dependent variable `pass` and the covariates `math` and `activity`. We put the latter ones in `Covariates` because they are continuous variables. Notice that `r jamovi()` recognizes `activity` as a nominal variable, because it lists only integer values. GAMLj would automatically transform it into a numerical variable and use it as covariate.

<img src="examples/gzlmmixed1/variables.png" class="img-responsive" alt="">

## The model

We need to specify the model, in terms of fixed effects and random effects. First, we expand the `Fixed Effects` tab and include the interaction into the model terms, by selecting both variables on the left panel and push them on the right.
...
The first table, `Model Info` recap the model call (the formula used in R), the description of the model (family and link function), and some overall model index. Here we can look at the R-squared, for datail see [technical details](gzlm_specs.html) and [R piecewise package implementation](https://cran.r-project.org/web/packages/piecewiseSEM/) 

<img src="examples/gzlmmixed1/results_modelinfo.png" class="img-responsive" alt="">

Based on the R-square indexes, we can see that our error of approximation of the data decreases of 4% ( ${R^2}_{marg}=.039$ ) thanks to the fixed effects, whereas all effects together decrease our error of approximation of 37% (${R^2}_{cond}=.378$). Put it in another way, our ability to 37% of the observed probabilities capturing also the variability of the intercepts.
...
For this model, with only intercepts as random coefficients across schools, the random component table is pretty simply. It shows the variance of the random intercepts. It is non-zero, so we are happy. It also show that the intercepts account for a large portion of variability ($ICC=.50$), indicating that schools are very different in the students' average probability of passing the exam.

## Omnibus test

<img src="examples/gzlmmixed1/results_omnibus1.png" class="img-responsive" alt="">

The omnibus (Wald) Chi-Squared test tests the main effects of the independent variables and their interaction. Notice that in GAMLj the continuous variables are centered by default, and thus we can interpret the linear effects of `math` and `activity` as _average effects_ or simply _main effects_ . Based on the p-value, our results seem to support an interaction and two main effects.

## Parameter estimates

The same information can be extracted from the parameter estimates table.

<img src="examples/gzlmmixed1/results_params1.png" class="img-responsive" alt="">
...
For generalized linear model, mixed included, a good strategy is to visualize the effects by plotting the predicted values. GAMLj plots the predicted values after transforming them back to the original scale of the dependent variable, in this case probability.
...
Because `activity` is a continuous variable, the separated lines will show the effect of `math` for three _interesting_ values of the moderator `activity`. The defaul in GAMLj is to show effect for the moderator set at `Mean-1SD`, `Mean`, and `Mean+1SD`. This default can be altered in the `Covariates scaling` tab.
...
Notice that the random effects depict different curves for different schools, even though the only random effect is the intercept. This is not weired in generalized linear models. The random intercept is estimated for the logit, so each school will have a different shape of the response. If we wish to appreciate how the random linear effects vary, we can ask for the `Linear Predictor` plot, which plots the effects in the logit scale.
