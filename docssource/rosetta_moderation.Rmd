...
For a longer discussion please check out `r cite_example("exercise")`.
...
Estimation and test for combination of  model parameters can be obtained in SPSS with `/LMATRIX` subcommand. `/LMATRIX` subcommand syntax is `/LMATRIX label term1 code1 term2 code2 etc `. In our case, `term1` is `cxage`, code1 is 1, term2 is the interaction `cxage*czexer` and code2 is one SD of `zexer`. 
...
We write build a dataset with all combinations of  interesting levels (SD and mean) of the independent variables.
...
Please notice that we estimated the centered model for the sake of caparability, but in PROCESS you do not need to center the variables because the macro will center them for you. PROCESS produces also the syntax for plotting the simple slopes, using the strategies we used in _pure SPSS_ section.
