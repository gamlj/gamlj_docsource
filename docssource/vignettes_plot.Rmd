...
To have additional plots, we can re-estimate the model using the options described above or to use `plot()`. The function `plot()` accepts either a formula or character options. The formula is a right hand side formula specifying the x-axis variable, the (optional) moderators.
...
Because the moderator variable `Petal.Length` is a continuous variable, the command plots the independent variable effect at three different levels of the moderator. By default the three levels are the mean and mean plus one standard deviation and the mean minus one standard deviation. The conditioning values can be changed with the option `simpleScale`. The option accepts mean_sd' (default) and 'percent'.
...
```{r warning=FALSE, message=FALSE}
plot(mod,formula = ~Sepal.Width*Petal.Length*Petal.Width)

```

# Character options

Alternatively, the function accepts three charcater options. The three options are:

* `haxis` defining the variable in the X-axis
* `sepLines` defining the variable whose levels define the way the effect of the variable in the X-axis is broken down (the moderator)
* `sepPlots` defining the variable whose levels define the way the two-way interaction is broken down

A two-way plot, for instance, can be obtained as follows:

```{r warning=FALSE, message=FALSE}
plot(mod,haxis="Sepal.Width", sepLines="Petal.Length")
```
...
```{r warning=FALSE, message=FALSE}
p<-plot(mod,haxis="Sepal.Width", sepLines="Petal.Length", plotRaw=T,plotError="ci")
p
...
data("schoolexam")
mod<-gamljGlmMixed(formula=pass~math+(1|school),data=schoolexam,plotHAxis = "math",plotRandomEffects=T)
p<-plot(mod, haxis = "math",plotRandomEffects=T)
```
