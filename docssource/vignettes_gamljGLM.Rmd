...
The object `mod' is an R6 object which contains all the results tables and some additional function that can be used in subsequent analyses. Tables are pretty printed, but they can be modified or manipulated by trasforming them in dataframes as follows:
...
The same analysis can be done by updating the model with the `gamlj_update()` function. Almost all the options available in `gamljGLM()` can be added to a model by running `gamlj_update(mod,...)` where `...` is any option or options accepted by `gamlj_update()`.


```{r }
mod2_2<-gamlj_update(mod1,effectSize = c("beta", "eta","partEta","omega","epsilon"))
mod2_2$main$anova
```
...
One can also report the betas (last column in the `Model Coefficients` table). The estimates of the betas are correct also in the presence of the interaction, because the variables are standardized before the interaction term is computed.
...
One can change the conditioning levels of the moderators with the `simpleScale` option (defaul is `mean_sd` for mean plus/minus one SD), either added to the `gamljGLM()` function or to the `gamlj_simpleEffects()` function. If one wants to use the percentiles (25%,50%,75%), for instance, one can run the following.
...
One can further tweak the appearance of the tables by selecting a different value/labels  in `simpleScaleLabels` option. Options are "labels", "values" and "values_labels". The latter output the values and the labels of the conditioning values.
...
```{r warning=FALSE, message=FALSE}
mod4<-gamljGLM(formula = yendu~xage*zexer, data=data, 
               plotHAxis = "xage",plotSepLines = "zexer",plotError = "ci",plotRaw = TRUE)
mod4$descPlot
```
