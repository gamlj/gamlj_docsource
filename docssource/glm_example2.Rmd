...
`r keywords("Polynomial regression, non linear effects, general linear model")`

`r version("1.0.5")`

In this example we exemplify polynomial regression (GLM) in jamovi GAMLj. Data are (simulated data) [here](https://github.com/mcfanda/gamlj/data/qsport.csv). 

# The dataset

The (simulated) dataset has three variables of interest. Imagine we measured athletes performace in a match using a standard scale and the number of hours they trained in a week. The idea is to study the relationship between hours of training and performance. Because training can bee good for performance but training too much may have detrimental effects on performance, we foresee a non-linear effect of training hours on performance. Furthermore, athletes are divided in two groups (variable `types`), professionals (`P`) and amateurs (`A`), and we want to check if the effects of training may be different in the two groups.
...
BY inspecting the F-tests and the estimates (B coefficients) we can see that we have a _linear_ and a _quadratic_ effect of `hours` to `performance`, whereas the _cubic_ effect ($hours^3$) is trivial and can be disregarded (the $\eta^2p$ is practically zero).

<img src="examples/glm2/results1_2.png" class="img-responsive" alt="">

When it comes to polynomial models, the best way to figure out the relationship between variables is to plot the effects. We can do that by selecting the `Plot` panel and by putting `hours` in the `Horizontal Axis` filed (mind that in GAMLj default the IV is centered to its mean, to obtain a nice plot I changed the IV scaling to `none` in `Covariates scaling` panel).
...
We can now analyze possible difference due the the type of athletes by introducing `type` as a factor in the model.
...
However, we want to see if the effects of `hours` depend on `type` so we need to include the interactions. We need to interactions: the interaction _linear hours_ by _type_, and _quadratic hours_ by _type_ (I removed _cubic hours_ based on the previous analysis).
...
The model info table shows the actual model we estimated and the $R^2$, the latter clearly larger than the $R^2$ of the previous model.
...
The simple effects tables show that for low (-1SD) and high (+1SD) training the groups are statistically different and the difference is to the advantage of the Professional group ($P-A=2.977$ and $P-A=2.667$), whereas for average training the performance does not seem to be different between the two groups ($P-A=-0.0403$). By changing the covariate conditioning in the `Covariate Scaling` panel one can test these difference for all values of `hours` that we wish.
...
In practice, the simple effects tests we have seen tested the difference between the two lines of the groups at -1, 0, and 1 levels of `hours`. Being `hours` standardized now, those values correspond to -1SD, mean, and +1SD of training hours.
