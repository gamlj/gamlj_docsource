...
R does not have an out-of-the-box contrast equivalent to `repeated` contrast. One can, howerver, uses the `contr.sdif()` function in `MASS` package. As compared with jamovi and SPSS the comparisons are in the opposite directions, so we simply multiply the code by -1 (t-tests and p-values do not depend on this):
